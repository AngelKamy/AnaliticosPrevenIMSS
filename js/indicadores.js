...
document.addEventListener('DOMContentLoaded', function () {
    // ... (selectores del DOM y variables de configuración como antes) ...
    ...
    // --- FUNCIONES DE INICIALIZACIÓN Y GRÁFICO PRINCIPAL (sin cambios relevantes aquí) ---
    function initSVG() {
        chartContainer.html('');
    ...
    function actualizarGrafico() {
        if (!selectedYear || !selectedMonth || !svg) {
            if(svg) {
        ...
        d3.select("#tituloGrafica")
            .text(etiquetasIndicadores[idx] || "Indicador de productividad");
    ...
    function crearEscalas(datos) {
        const padding = datos.length > 30 ? 0.05 :
                       datos.length > 20 ? 0.1 :
                       datos.length > 10 ? 0.15 : 0.2;

        yScale = d3.scaleBand()
            .domain(datos.map(d => d.unidad))
            .range([0, height])
            .padding(padding);

        xScale = d3.scaleLinear()
            .domain([0, d3.max(datos, d => d.valor) * 1.1 || 10])
            .range([0, width])
            .nice();
    }
    ...
    function dibujarEjes(datos) {
        ...
        svg.append("g")
            ...
            .selectAll("text")
                .style("font-family", "Montserrat, sans-serif")
                .style("font-size", () => {
                    if (datos.length > 35) return "8px";
                    if (datos.length > 25) return "9px";
                    if (datos.length > 15) return "10px";
                    return "11px";
                })
    ...
    function dibujarBarras(datos, config, idx) {
        ...
        bars.enter()
            ...
            // MODIFICACIÓN: Pasar selectedYear y selectedMonth a mostrarGraficoHistorico
    ...
    function dibujarLeyenda(config) {
        ...
        const leyendaData = [
            { color: config.backgroundColors[0], text: `> ${config.umbralSuperior}` },
            { color: config.backgroundColors[1], text: `${config.umbralInferior} - ${config.umbralSuperior}` },
            { color: config.backgroundColors[2], text: `< ${config.umbralInferior}` }
        ];
    ...
    function mostrarTooltip(event, d, idx, config) {
        ...
        tooltip.html(`
            ...
            <div style="font-size:0.9em;">${etiquetasIndicadores[idx]}</div>
    ...
    function cargarMeses() {
       ...
       if (!selectedMonth || !sortedMeses.includes(selectedMonth)) {
             selectedMonth = sortedMeses[sortedMeses.length - 1]; // Esta línea seleccionará el ÚLTIMO mes por defecto
        }
    ...
    // --- FUNCIONES PARA GRÁFICO HISTÓRICO ---
            ...
            if (monthsInYearData) {
                monthOrder.forEach(monthName => {
                    if (monthsInYearData[monthName]) {
                        const unitDataArray = monthsInYearData[monthName];
                        const unitDataObject = unitDataArray.find(u => u.unidad === unitName);
    ...
    // MODIFICACIÓN: Aceptar yearOfContext y monthOfContext
    function mostrarGraficoHistorico(unitName, yearOfContext, monthOfContext) {
        ...
        // Encontrar el índice inicial basado en yearOfContext y monthOfContext
        let initialIndex = currentHistoricalData.length - 1; // Por defecto al último si no se encuentra
        const foundIndex = currentHistoricalData.findIndex(d => d.originalYear === yearOfContext && d.originalMonth === monthOfContext);

        if (foundIndex !== -1) {
            initialIndex = foundIndex;
        } else {
            // Si no se encuentra el mes exacto, podríamos buscar el más cercano o el último del año de contexto.
            // Por ahora, si no se encuentra, se queda en el último punto de toda la serie histórica.
            // Opcional: buscar el último punto del yearOfContext si monthOfContext no coincide.
            const lastIndexOfYear = currentHistoricalData.map(d => d.originalYear).lastIndexOf(yearOfContext);
            if(lastIndexOfYear !== -1) {
                 // Esto es un poco más complejo porque necesitaríamos el último mes de ese año *en los datos*.
                 // Simplificación: si el mes no coincide pero el año sí, buscamos el último punto de ese año.
                 let tempIndex = -1;
        ...
        if (timeSlider.node()){
            ...
            timeSlider.node().value = initialIndex; // Establecer el valor inicial del slider
        }
        
        dibujarGraficoHistoricoD3(currentHistoricalData);
        actualizarPuntoDeslizanteYBotones(initialIndex); // Actualizar al punto inicial
    }

    function dibujarGraficoHistoricoD3(hData) {
        historicalChartDiv.html('');
        if (hData.length === 0) {
            historicalChartDiv.append("p").text("No hay datos para graficar.").style("text-align", "center").style("padding-top", "50px");
            return;
        }
        ...
        xHistoricalScale = d3.scalePoint()
            .domain(hData.map(d => d.periodoDisplay))
            .range([0, hWidth])
            .padding(0.5);

        const yMin = d3.min(hData, d => d.valor);
        const yMax = d3.max(hData, d => d.valor);
        ...
        historicalSvg.append("path").datum(hData.filter(d => d.valor != null && !isNaN(d.valor)))
            .attr("fill", "none").attr("stroke", "#007bff").attr("stroke-width", 2.5).attr("d", historicalLine);
    ...
    function actualizarPuntoDeslizanteYBotones(index) {
        ...
        // Asegurarse que dataPoint existe antes de intentar acceder a sus propiedades
        if (!dataPoint) {
            if(slidingDot) slidingDot.style("opacity", 0);
        ...
        if (dataPoint.valor !== null && !isNaN(dataPoint.valor) && xHistoricalScale && yHistoricalScale && slidingDot) {
            ...
            const pointConfig = configuracionIndicadores[dataPoint.originalYear]?.[dataPoint.originalMonth]?.[selectedIndicator] || {};
